...
Edge::~Edge()
{
	// TODO: this may not always work / unsafe???
	std::list< double * >::iterator edgePointCoordinatesIt;
	for(edgePointCoordinatesIt = edgePointCoordinates.begin(); edgePointCoordinatesIt != edgePointCoordinates.end(); ++edgePointCoordinatesIt)
	{
		delete *edgePointCoordinatesIt;
	}
...
ClosedSurface::ClosedSurface ( PolyDataPointerType mesh ) : Surface ( mesh )
{
	...
	double tol = 1e-6;
	insideSurfaceFilter->SetTolerance(tol);
