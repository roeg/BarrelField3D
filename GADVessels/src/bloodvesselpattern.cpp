...
BloodVesselPattern::BloodVesselPattern(const char * inputFilename, const char * outputFilename)
{
	idCounter=0;
	vesselNr=0;
	this->inputFilename = inputFilename;
	this->outputFilename = outputFilename;
	readImage(1, 1, inputFilename);
}

BloodVesselPattern::BloodVesselPattern(const char * inputFilename, const char * outputFilename, int start, int stop)
{
	idCounter=0;
	vesselNr=0;
	this->inputFilename = inputFilename;
...
void BloodVesselPattern::readImage( int start, int stop, const char * inputFilename )
{
...
std::list<std::vector<float> >  BloodVesselPattern::GetAmiraEdge(std::vector<float> &vessel)
{
  ...
  for (int i=1;i<=Res;i++)
  {
	...
	contour_point.push_back((vessel[VESSEL_X]+cos(angle)*(vessel[VESSEL_DIAM]/2.0))*XYSAMPLING);
	contour_point.push_back((vessel[VESSEL_Y]+sin(angle)*(vessel[VESSEL_DIAM]/2.0))*XYSAMPLING);
...
#include <list>
int isThereAlreadyAVesselNextToThis(int posX, int posY, int diameter, std::list<Vessel> &vesselList)
{
	std::list<Vessel>::iterator pos;
...
int
BloodVesselPattern::
detectBloodVessel(itk::Image< CalcPixelType >::Pointer img, int threshold_)
{
...
// 		int threshold = 100;
// 		int threshold = 50;
// 	int threshold=65;//Leica Florida data
// 	int threshold=13;//Leica Florida data normalized to max gray value of 53
// 	int threshold=45;//Leica Florida data normalized to max gray value of 181
	int threshold=20;//old Leica Heidelberg data
	...
	// only for projection image without border:
	int distanceToBoundary=(int) sqrt((float) Height*Width)/10;
// 	int distanceToBoundary=(int) sqrt((float) Height*Width)/20;
...
#endif
								Vessel newVesselObj(posX, posY, vesselDiamter);
								vesselList.push_back(newVesselObj);
								
								std::vector<float> newVessel;
								newVessel.push_back(posX);
								newVessel.push_back(posY);
								float posZ=0.0;
								newVessel.push_back(posZ);
								newVessel.push_back(vesselDiamter);
								vesselNr++;
...
void BloodVesselPattern::ExtractBloodVesselPattern(int threshold)
{
...
// 	BloodVesselPattern * vesselPattern= new BloodVesselPattern();
	...
	for(int z = tmpIndex[2]; z < tmpSize[2]; ++z)
	{
		...
		CalcImageType::RegionType planeRegion;
		CalcImageType::SizeType planeSize;
		CalcImageType::IndexType planeIndex;
		
		planeSize[0] = tmpSize[0];
		planeSize[1] = tmpSize[1];
		planeSize[2] = 1;
		planeIndex[0] = tmpIndex[0];
		planeIndex[1] = tmpIndex[1];
		planeIndex[2] = z;
		
		planeRegion.SetIndex(planeIndex);
		planeRegion.SetSize(planeSize);
		
		CalcIteratorType originalIter( tmpImage, planeRegion );
		CalcIterator2DType projectionIter( projectionImage, projectionImage->GetLargestPossibleRegion() );
		
		for( originalIter.GoToBegin(), projectionIter.GoToBegin() ; !originalIter.IsAtEnd() && !projectionIter.IsAtEnd() ; ++originalIter, ++projectionIter )
		{
		...
		this->detectBloodVessel(projectionImage, threshold);
	}
