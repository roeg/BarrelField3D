...
int main( int argc , char * argv[])
{
	if(argc == 2)
	{
		...
		hocReader->readObjectLineFile();
		AmiraSpatialGraph * objectSG = hocReader->getSpatialGraph();
		std::string ofName(ifName);
		Reader * amWriter = new Reader(ofName.c_str(), ofName.c_str());
		amWriter->setSpatialGraph(objectSG);
